...
function redirectToSpotifyLogin() {
  const clientId = "22b5867be4c74e949ac5c0e10f6b1b12"; // your client ID
...
window.onload = () => {
  ...
  if (tokenFromUrl) {
    ...
    // Remove the hash safely (keeps you on the same page)
...
// === Display playlist summary + table ===
function displayPlaylist(playlist) {
  ...
  tracks.forEach((item) => {
    ...
    artworkCell.innerHTML = `<img src="${
      track.album.images[0]?.url || ""
    }" alt="Cover" style="width:60px; border-radius:4px;">`;
    artworkCell.style.padding = "10px";

    const titleCell = document.createElement("td");
    titleCell.innerHTML = `<a href="${
      track.external_urls.spotify
    }" target="_blank" style="color:#1DB954; text-decoration:none;">${
      track.name
    }</a>`;
